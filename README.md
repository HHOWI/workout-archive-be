...
- **장소 관리**: 자주 방문하는 운동 장소 등록 및 관리
  - 카카오맵 API 연동을 통한 위치 선택
  - 장소별 정보 저장 및 조회
- **장소 팔로우**: 특정 운동 장소 팔로우 기능
  - 장소별 운동 기록 카운트 제공
  - 장소별 피드 조회 가능
...
- **시각화 대시보드**: Chart.js와 React Chart.js를 활용하여 통계 데이터를 그래프로 시각화
  ...
  - **웨이트 기록 변화**: 특정 운동에 대한 최대 중량(1RM 추정치 포함), 총 볼륨 등의 변화를 라인 차트로 시각화
...
- **통합 검색**: 사용자, 운동 장소, 운동 종류에 대한 통합 검색
- **실시간 검색 제안**: 입력에 따른 실시간 검색 결과 제안
- **검색 결과 필터링**: 카테고리별 검색 결과 분류
...
- **라우팅 및 접근 제어**:
  ...
  - `ProtectedRoute`: 인증된 사용자만 접근 가능한 페이지 보호
  - `LoginGuard`: 로그인된 상태에서 접근할 수 없는 페이지 처리 (로그인, 회원가입 페이지 등)
